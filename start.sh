...
podman-chroot 'runuser -u builder -- bash -c "git clone --quiet https://github.com/tuna-os/fisherman /buildhome/fisherman && cd /buildhome/fisherman && git apply /app/patches/var-tmp-no-tmpfs.patch && git apply no-pull-on-live-iso.patch"'
...
podman-chroot 'pacman -Rns --noconfirm gnome-keyring valent flatpak cups cups-browsed samba smbclient tuned tuned-ppd ddcutil fprintd gpu-screen-recorder ttf-arphic-uming ttf-baekmuk wqy-microhei ttf-croscore ttf-droid gnu-free-fonts powertop libva-intel-driver bazaar flatseal'
...
# pull image to ISO
podman-chroot "podman pull ghcr.io/tartaria-dev/tartaria:stable-${ISO_NAME#*-}"
